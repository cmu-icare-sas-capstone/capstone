...
df = df.dropna(how='all',axis=1) # 43 columns dropped
...
# recommend_df = df[df["Comment"].str.contains(pattern)]
...
for comment in comments:
    ...
    import pickle
    pickle.dump(processed_comments, open('model/processed_comments.pkl', 'wb'))
...
# Two different vector representations of the same text data
# Dimensions of both vectorizers should match

# term-frequency matrix
# Each element in the matrix represents the number of times a given word appears in a given comment.
...
import pickle
pickle.dump(svdMatrix, open('model/svdMatrix.pkl', 'wb'))
...
pickle.dump(tsne_lsa_vectors, open('model/tsne_lsa_vectors.pkl', 'wb'))
